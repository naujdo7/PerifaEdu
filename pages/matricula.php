<?php
/* ─────────────────────────────────────────
   1. PRIMEIRO: PHP puro — sem nenhum HTML antes
   O header.php usa session_start(), então tudo
   PHP deve vir aqui antes de qualquer saída.
───────────────────────────────────────── */
$cursos = [
  'tecnico-de-ti' => [
    'nome'       => 'Técnico de TI',
...
$curso = isset($cursos[$slug]) ? $cursos[$slug] : $cursos['tecnico-de-ti'];
if (!isset($cursos[$slug])) $slug = 'tecnico-de-ti';

/* 2. SEGUNDO: inclui o header — session_start() roda aqui,
   ainda sem nenhum HTML impresso                          */
...
<head>
  ...
  <style>
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    body { font-family: 'Inter', sans-serif; background: var(--bg); color: var(--gray-700); min-height: 100vh; }

    .course-hero { background: linear-gradient(135deg, var(--blue-dark) 0%, #1e4db7 60%, #2563eb 100%); padding: 52px 32px 40px; color: #fff; }
    .course-hero .breadcrumb { font-size: .82rem; opacity: .7; margin-bottom: 18px; }
    .course-hero .breadcrumb a { color: #93c5fd; text-decoration: none; }
    .course-hero .breadcrumb a:hover { text-decoration: underline; }
    .course-hero-inner { max-width: 900px; margin: 0 auto; display: grid; grid-template-columns: 1fr auto; gap: 32px; align-items: center; }
    .course-tag { display: inline-block; background: rgba(255,255,255,.18); border: 1px solid rgba(255,255,255,.3); border-radius: 20px; font-size: .78rem; font-weight: 600; letter-spacing: .5px; padding: 4px 14px; margin-bottom: 14px; text-transform: uppercase; }
    .course-hero h1 { font-family: 'Poppins', sans-serif; font-size: 2.1rem; font-weight: 700; line-height: 1.2; margin-bottom: 14px; color: white; }
    .course-hero p.desc { font-size: 1rem; line-height: 1.7; opacity: .88; max-width: 580px; }
    .course-meta { display: flex; flex-wrap: wrap; gap: 20px; margin-top: 26px; }
    .meta-item { display: flex; flex-direction: column; gap: 2px; }
    .meta-item .label { font-size: .72rem; font-weight: 600; letter-spacing: .8px; text-transform: uppercase; opacity: .65; }
    .meta-item .value { font-size: .95rem; font-weight: 600; }
    .course-badge { background: rgba(255,255,255,.12); border: 2px solid rgba(255,255,255,.25); border-radius: 20px; padding: 28px 32px; text-align: center; backdrop-filter: blur(8px); min-width: 180px; }
    .course-badge .icon { font-size: 3rem; margin-bottom: 8px; }
    .course-badge .badge-label { font-size: .8rem; opacity: .7; font-weight: 500; }
    .course-badge .badge-value { font-family: 'Poppins', sans-serif; font-size: 1.15rem; font-weight: 700; margin-top: 4px; }

    .section-info { max-width: 900px; margin: 40px auto; padding: 0 24px; display: grid; grid-template-columns: 1fr 1fr; gap: 24px; }
    .info-card { background: var(--white); border-radius: var(--radius); padding: 28px; box-shadow: var(--shadow); }
    .info-card h3 { font-family: 'Poppins', sans-serif; font-size: 1rem; font-weight: 700; color: var(--blue-dark); margin-bottom: 16px; display: flex; align-items: center; gap: 8px; }
    .info-card ul { list-style: none; display: flex; flex-direction: column; gap: 10px; }
    .info-card ul li { font-size: .9rem; display: flex; align-items: flex-start; gap: 10px; line-height: 1.5; }
    .info-card ul li::before { content: '✓'; color: var(--green); font-weight: 700; flex-shrink: 0; margin-top: 1px; }
    .req-list li::before { content: '◆'; color: var(--blue-mid); font-size: .7rem; margin-top: 4px; }

    .divider { max-width: 900px; margin: 8px auto 32px; padding: 0 24px; }
    .divider hr { border: none; border-top: 2px solid var(--blue-light); }
    .divider-title { font-family: 'Poppins', sans-serif; font-size: 1.5rem; font-weight: 700; color: var(--blue-dark); margin-bottom: 6px; }
    .divider-sub { font-size: .9rem; color: var(--gray-500); }

    .form-wrapper { max-width: 900px; margin: 0 auto 60px; padding: 0 24px; }
    .form-card { background: var(--white); border-radius: var(--radius); padding: 40px 48px; box-shadow: var(--shadow); }
    .form-section-title { font-family: 'Poppins', sans-serif; font-size: .85rem; font-weight: 700; color: var(--blue-mid); text-transform: uppercase; letter-spacing: 1px; margin: 28px 0 16px; padding-bottom: 8px; border-bottom: 2px solid var(--blue-light); }
    ...
    input[type="text"], input[type="date"], input[type="number"], select, textarea {
      width: 100%; padding: 11px 14px; border: 1.5px solid var(--gray-300); border-radius: 9px;
      font-family: 'Inter', sans-serif; font-size: .9rem; color: var(--gray-700);
      background: var(--gray-100); transition: border-color .2s, box-shadow .2s, background .2s; outline: none;
    }
    input:focus, select:focus, textarea:focus { border-color: var(--blue-mid); background: #fff; box-shadow: 0 0 0 3px rgba(37,99,235,.12); }
    ...
    .checkbox-group input[type="checkbox"] { width: 18px; height: 18px; flex-shrink: 0; margin-top: 2px; accent-color: var(--blue-mid); }
    .checkbox-group label { font-size: .85rem; font-weight: 400; color: var(--gray-500); line-height: 1.5; }
    .checkbox-group label a { color: var(--blue-mid); text-decoration: none; }

    .btn-submit { width: 100%; padding: 15px; background: linear-gradient(135deg, var(--blue-mid), #1d4ed8); color: #fff; font-family: 'Poppins', sans-serif; font-size: 1rem; font-weight: 700; border: none; border-radius: 10px; cursor: pointer; transition: opacity .2s, transform .1s; margin-top: 28px; letter-spacing: .3px; }
    .btn-submit:hover { opacity: .93; }
    .btn-submit:active { transform: scale(.99); }
    .btn-submit:disabled { opacity: .6; cursor: not-allowed; }

    .alert { padding: 16px 20px; border-radius: 10px; font-size: .9rem; font-weight: 500; display: none; margin-top: 20px; align-items: flex-start; gap: 12px; }
    ...
    <div class="course-hero-inner">
      <div>
        ...
        <div class="course-meta">
          <div class="meta-item"><span class="label">Duração</span><span class="value">⏱ <?= htmlspecialchars($curso['duracao']) ?></span></div>
          <div class="meta-item"><span class="label">Turno</span><span class="value">🌙 <?= htmlspecialchars($curso['turno']) ?></span></div>
          <div class="meta-item"><span class="label">Vagas</span><span class="value">👥 <?= htmlspecialchars($curso['vagas']) ?></span></div>
          <div class="meta-item"><span class="label">Certificado</span><span class="value">🎓 <?= htmlspecialchars($curso['certificado']) ?></span></div>
        </div>
      </div>
    ...
    <form id="matriculaForm" novalidate>
      ...
      <div class="form-row">
        ...
        <div class="form-group">
          ...
          <input type="number" id="idade" name="idade" placeholder="Calculada automaticamente" min="14" max="99" readonly style="background:#e9effd;cursor:not-allowed;">
        </div>
        <div class="form-group">
      ...
      <div class="form-row">
        <div class="form-group">
          ...
          <select id="escolaridade" name="escolaridade">
            ...
            <option value="medio_incompleto_cursando">Médio Incompleto (cursando)</option>
            <option value="medio_incompleto_interrompido">Médio Incompleto (interrompido)</option>
            <option value="medio_completo">Médio Completo</option>
            <option value="superior_incompleto_cursando">Superior Incompleto (cursando)</option>
            <option value="superior_incompleto_interrompido">Superior Incompleto (interrompido)</option>
      ...
      <button type="submit" class="btn-submit" id="btnSubmit">✅ Enviar Inscrição</button>

      <div class="alert alert-success" id="alertSuccess">
        <span class="alert-icon">🎉</span>
        <div class="alert-text">
          <strong>Inscrição enviada com sucesso!</strong>
          <p>Recebemos sua matrícula para o curso <strong><?= htmlspecialchars($curso['nome']) ?></strong>. Entraremos em contato em breve.</p>
        </div>
      </div>
...
document.getElementById('matriculaForm').addEventListener('submit', function(e){
  ...
  const cpfOk = validCPF(document.getElementById('cpf').value);
